...
Watches a list of profile pages and records when each one goes online or offline. A profile counts as offline while its page shows a text you configure, and as online otherwise. Every change is appended to a per-profile log along with the profile's location, a screenshot is captured when a profile comes online, and each round's changes can be emailed through [Resend](https://resend.com).
...
5. If `location_selector` is configured, read the text of the first element it matches as the location, whether the profile is online or offline.

Each check prints a line such as `2026-09-19 16:28:53 alice → online (Paris)`, the location in parentheses only when one was found.
...
| `site.location_selector` | no | CSS selector of the element holding the profile's location, read on every check. `:has()` works, so `div:has(> i.fa-map-marker-alt)` selects the element next to a map marker icon |
...
| Path | Content |
| --- | --- |
| `$XDG_DATA_HOME/profile-availability/<profile>.txt` | One line per status change: `YYYY-MM-DD HH:MM:SS <status> <location>`, the location only when `location_selector` found one |
